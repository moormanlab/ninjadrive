    ...
    difference(){
      union(){
        ...
        hull(){
            ...
            rotate([0,0,-30+i*120])translate([lengthT/2,0,0])cylinder(h=heightT,d=widthHub);
        }
      ...
      mirror([i,0,0])
      difference(){
        translate([0,-0.25,heightT-knotH])rotate([0,0,-150])cube([widthHub,widthHub,knotH+.1]);
        translate([-widthHub-.1,-widthHub-0.25,heightT-knotH])cube([widthHub,widthHub,knotH+.1]);
      }

      //holes
      for (i = [-1,1])
        rotate([0,0,i*30])
        translate([-i*lengthT/2,0,-.05])cylinder(h=heightT+.1,d=tubeD);
    ...
    difference(){
      union(){
        ...
        hull(){
            ...
            rotate([0,0,-30+i*120])translate([lengthT/2,0,0])cylinder(h=heightS,d=widthS);
        }
        for (i = [1,2,3])
            rotate([0,0,-30+i*120])translate([lengthT/2,0,0])cylinder(h=3,d=widthS);
      }

      //holes
      for (i = [1,2,3])
        rotate([0,0,-30+i*120])
        translate([lengthT/2,0,-.05])cylinder(h=heightS+1+.1,d=tubeD);
      translate([0,0,-.05])cylinder(h=heightS+.1,d1=tubeD-0.05,d2=tubeD);
      translate([0,0,2.0001])cylinder(h=1+.1,d=washerD);
      translate([0,0,2.0001])rotate([0,0,30])cylinder(h=1+.1,d=washerD*1.9,$fn=3);
    }
}
...
module cover(){
  ...
  difference(){
    ...
    for(i=[0,1])
      mirror([i,0,0])translate([topOut/2,0,h+dy/2]) rotate([0,90,0])cylinder(h=6,d=1.1,$fn=40,center=true);
  }
  for (j=[0,1])
   mirror([1*j,0,0])
   translate([(baseIn+baseOut)/4,0,dh])
...
module extendedCover(){
    fn1=80;
    dy = dyCover; // overrrides the global dy
    extensionH= 5;
    cover();
    difference() { 
      translate([0,0,h+dy])cylinder(h=extensionH,d=topOut+(topOut-baseOut)*dy/(h-dh),$fn=fn1);
      translate([0,0,h+dy-.05])cylinder(h=extensionH+.1,d=topIn+(topIn-baseIn)*dy/(h-dh),$fn=fn1);
      translate([0,-10+0.1,h+dy+extensionH/2])cube([30,20,extensionH+.01],center=true);
    }
}
...
*union(){
 color("Red") 
   extendedCover();
 color("Blue") 
   rotate([0,0,180.01]) extendedCover();
 translate([0,0,12]) rotate([0,180,0]) stage();
 body();
 //cap();
